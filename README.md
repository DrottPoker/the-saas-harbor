...
- Demo products while the directory is new: until 12 real products share verified MRR, the leaderboard, Browse and New arrivals fill their first page with made-up products after the real ones. They are marked Demo everywhere, never ranked or called verified, have their own pages under `/demo` that search engines skip, and disappear as real products join. `DEMO_PRODUCTS=off` hides them at once.
...
- Site statistics, counted without cookies: an Analytics page in the admin panel (`/admin/analytics`) with visitors, visits and page views over 24 hours to 12 months, the most viewed pages, sources and campaigns, countries, devices, browsers and operating systems, from our own database; and Vercel Web Analytics in production, read in the Vercel dashboard. Bots, admin pages and signed-in admins are left out, and addresses keep only their campaign tags.
...
`npm run dev` generates the server-only secrets in `.env.local`: `SUPABASE_SECRET_KEY` (the local service-role key), `STRIPE_KEY_ENCRYPTION_KEY` and `CRON_SECRET`. The encryption key is kept once created; replacing it makes stored provider keys unreadable, and makers would have to reconnect. Production needs the same three secrets with new values, `REVENUE_ALLOW_TEST_KEYS` unset, the SMTP variables for Resend, and the two scheduled jobs, which the database runs: `/api/revenue/sync` every ten minutes and `/api/email/send` every minute, with `Authorization: Bearer $CRON_SECRET` (see Production).
...
- **App:** Vercel, functions in Frankfurt (`fra1`), deployed from `main`. `vercel.json` sets the framework (a project created with the preset Other otherwise looks for a static `public` folder and fails), the region, and turns off deployments of `development`. The Hobby plan is enough to start, since the scheduled jobs run in the database; Vercel allows Hobby only for non-commercial use, so move to Pro once the site earns money. Web Analytics is turned on under the project's Analytics tab; the app renders its script only when `VERCEL_ENV` is `production`, so previews, local servers and tests send nothing.
- **Scheduled jobs:** pg_cron in the database calls `POST /api/email/send` every minute and `POST /api/revenue/sync` every ten minutes through pg_net, with `Authorization: Bearer` and the production `CRON_SECRET` (migration `20260926010000_scheduled_jobs.sql`). Where to call and the secret are in Supabase Vault; without them, as locally and in CI, a run does nothing.
