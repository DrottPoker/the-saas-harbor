...
- Easy to find: public pages arrive complete, with their title, description and canonical link in the head, so crawlers that do not run JavaScript (AI assistants among them) read everything, and a missing page answers 404. The home page describes the organization and its logo, and the leaderboard as a list. Later pages of a list are pages of their own, searches stay out of search results, and so do empty profiles. A favicon file, an Apple touch icon and a 512-pixel logo come from the same drawing as the site's logo. In production, new, changed and deleted product pages are announced at once to Bing and the other search engines that take IndexNow notices, and the `vercel.app` address redirects to the domain.
...
`npm run dev` generates the server-only secrets in `.env.local`: `SUPABASE_SECRET_KEY` (the local service-role key), `STRIPE_KEY_ENCRYPTION_KEY` and `CRON_SECRET`. The encryption key is kept once created; replacing it makes stored provider keys unreadable, and makers would have to reconnect. Production needs the same three secrets with new values, `REVENUE_ALLOW_TEST_KEYS` unset, the SMTP variables for Resend, and the scheduled jobs, which the database runs: `/api/revenue/sync` every ten minutes, `/api/email/send` every minute and `/api/domains/check` every hour, with `Authorization: Bearer $CRON_SECRET` (see Production).
...
**Search engines.** Production redirects every other host, such as the project's `vercel.app` address, to `NEXT_PUBLIC_SITE_URL` (except `/api/`, so the scheduled jobs work whatever address they call), and announces product pages through IndexNow when founders save or delete them (`src/lib/indexnow.ts`; the key is public and served at `/indexnow.txt`). Once, the owner adds `thesaasharbor.com` to Google Search Console as a Domain property, verified with the DNS TXT record it gives, and submits `https://thesaasharbor.com/sitemap.xml`; then imports the site into Bing Webmaster Tools from Search Console. Both report what is indexed, errors and the searches that find the site. Bing's index also serves ChatGPT search, Copilot and DuckDuckGo. The home page and a product page can be checked with Google's Rich Results Test.
