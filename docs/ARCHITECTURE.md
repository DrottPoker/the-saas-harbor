...
`scripts/test-local.mjs` obtains credentials from the local CLI without printing them, refuses a non-loopback API, and starts Playwright with a separate `.next-e2e` output. Its admin credential exists only in the test process. Email stays inside local Mailpit. Tests remove their users and uploaded files afterward. Database access tests run inside a rolled-back transaction and have no persistent test fixtures.
