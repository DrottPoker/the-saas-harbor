...
At the project owner's request, sign-up now has a required box: "I am at least 16 and agree to the Terms of Service. I have read the Privacy Policy." The server refuses a sign-up without it, and the accepted version and time are recorded per account in `private.terms_acceptances` (migration `20260926020000_terms_acceptance.sql`), which account deletion removes; the privacy policy says so. The terms page is now called Terms of Service, and at the owner's choice the age limit is 16 (it was 18), Swedish law applies with the Swedish general courts, and a liability section limits liability for indirect losses and for what users publish, without touching consumer rights. The privacy policy has a matching Age section. Compared with TrustMRR's terms and privacy policy (read 2026-09-25), ours are milder: TrustMRR excludes liability for direct losses too, has users indemnify it, applies changed terms through continued use, and bans permanently, while ours notify users before changes and let them appeal decisions. The owner's production account predates the box and has no record. The browser test checks the box, the server's refusal and the recorded version; the whole path was also checked locally through Auth.
