...
At the project owner's request, after a look at PublishYourSaaS, founders can prove that the product's website is theirs. Anyone could list a known product's address before and connect their own payment account; now the real owner can show the domain is theirs. The product editor has a Domain section with the TXT record to add (`_thesaasharbor.<domain>`, with a code for the product) and a Check DNS button; the product page then says, under the revenue line, that the founder proved control of the domain and when it was last checked, and the Markdown version lists it. TrustMRR has no such check: it ties a startup only to its payment account.
