...
The record is looked up again every day by a new scheduled job (`/api/domains/check`, hourly); a record missing for three days, or a website on another domain, removes the mark. Founders check at most once every ten seconds per product and 30 times an hour. Only the server records a result, with the service-role key (migration `20260926100000_domain_verification.sql`). pgTAP (`domain_verification.test.sql`), unit tests (the domain of an address, the lookup) and a browser test against a local DNS server cover it, with Axe scans of the editor in both themes and a check for sideways scrolling at 390 px.
