...
## Screenshots 2026-09-26

At the project owner's request, product pages show a screenshot of the website's landing page, the way TrustMRR does: the app takes it with headless Chromium soon after a product is listed or its website changes, and again every 30 days. The product page shows the top of it above About, with the date and a link to the full page (up to 4,096 pixels tall); the editor has a Screenshot section with the image, how it is coming along or why the last attempt failed, and Take a new screenshot (at most every ten minutes). Founders can turn it off under Visibility. The Markdown versions and the structured data include it.

A new scheduled job (`/api/screenshots/capture`, every five minutes) takes the screenshots that are due and stores them in the founder's folder in `profile-images`, so deleting the product or the account deletes them (migration `20260926110000_screenshots.sql`, applied to production on 2026-09-26, before the code; the jobs call routes the live site does not have yet, which answer 404 until the code is deployed). The browser reaches only public addresses: every request goes through a proxy in the app that checks the address it connects to. Locally it runs Playwright's Chromium; on Vercel `@sparticuz/chromium`, a new optional dependency, with `playwright-core` now a regular one. pgTAP (`screenshots.test.sql`), unit tests (which addresses the browser may reach) and a browser test that screenshots a landing page served by the fake provider server cover it, with Axe scans of the product page and the editor in both themes and a check for sideways scrolling at 390 px.
...
22. Screenshots have only been taken locally (Windows, Playwright's Chromium). The first on Vercel will show whether `@sparticuz/chromium` fits the function as configured (`outputFileTracingIncludes`, Node.js 22.17 or later, 120 seconds); until then the product pages simply show none, and the editor shows the failure. Sites that block automated browsers or show a cookie banner are captured as they appear; the founder can turn the screenshot off.
