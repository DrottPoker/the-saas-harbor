...
2. Legal pages. The privacy policy and the terms are drafts: set the operator's name and contact address in `src/lib/legal.ts`, check the named providers (Vercel, Supabase, Resend) and the transfer safeguards for Vercel and Resend, and have both reviewed, including the age limit, liability and governing law, which the terms leave out. The contact address is also where reports from people without an account and appeals go.
...
4. Production Auth settings: all of them are in `supabase/config.toml` with `[remotes.production]` (the email templates and subjects, `https://thesaasharbor.com` as site URL and its `/auth/confirm` as redirect URL, 60 seconds between emails to one address, `secure_password_change`, a 12-character minimum password) and are applied with `npm run auth:production` once the Resend domain is verified (see README, Production). Still open: email rate limits to match Resend's plan and the CAPTCHA decision (see Release review).
...
6. Production hosting: Vercel (Pro, for per-minute jobs and commercial use), Supabase Cloud (Pro before launch) and Resend, set up as in README, Production, with SPF, DKIM and DMARC for `thesaasharbor.com`. After the first deployment, check that both scheduled jobs run in Vercel's Cron Jobs view.
