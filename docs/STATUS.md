...
- **Pages arrived as a loading placeholder.** The root `loading.tsx` made every page stream: the HTML held a Loading placeholder in `<main>` and the content in hidden blocks at the end, which a script moves into place. Crawlers that do not run JavaScript, such as those of ChatGPT, Claude and Perplexity, and text extractors that drop hidden elements, saw little more than the placeholder. Public pages now have no `loading.tsx` and arrive complete; the dashboard, messages and the admin panel keep the placeholder. Moving between public pages now waits for the next page (0.3 to 0.5 seconds on the live site, measured from Sweden) instead of showing the placeholder first.
- **Metadata was in the body.** Next.js streams the title, description and canonical link into the body for every user agent except a list of known bots (Bingbot among them, not Googlebot or the AI crawlers). Google reads it once it has rendered the page and a script has moved it into the head; crawlers that do not render miss it. `htmlLimitedBots: /.*/` now puts it in the head of every response.
- **Missing pages answered 200.** A missing product, profile, category or technology now answers 404, and a renamed product's earlier address a real 308, since nothing is streamed before the page knows. The HTML of a 404 is then Next.js's bare error document, and the browser draws the not-found page, with the header, footer and saved theme, once the scripts load; before, the header and footer came first and the not-found text followed in the same way.
- **A second copy of the site** answered at `the-saas-harbor.vercel.app`. Production now redirects every other host to `NEXT_PUBLIC_SITE_URL`, except `/api/` for the scheduled jobs.
- **No favicon file, Apple touch icon or organization data.** `/favicon.ico` (16, 32 and 48 pixels), the Apple touch icon and a 512-pixel `/logo.png` are drawn from the logo, and the home page describes the organization, its logo and the site as one graph, and the leaderboard's first page as a list.
- **Titles and descriptions.** Browse, New arrivals, How it works, Privacy and Terms had the site's sharing title and some the site's description; every public page now sets its own. The home page is titled Free SaaS directory and verified MRR leaderboard, with a description that says listing is free and how to reach the leaderboard; Browse is Browse SaaS products and New arrivals New SaaS products in titles (the headings are unchanged). Search results pages are `noindex, follow`, later pages of a list are pages of their own, and profiles without products, a headline and an About section stay out of search results. The root layout allows large image previews and snippets of any length.
- **IndexNow.** Production announces product pages to Bing, Yandex, Seznam, Naver and the other IndexNow search engines when founders save or delete them. Bing's index also serves ChatGPT search, Copilot and DuckDuckGo.
- **llms.txt** now says that listing is free, without payment details, that connecting a payment provider is optional and ranks the product, and where founders sign up.

Verified with a production build served locally: the same pages fetched as GPTBot and Googlebot have the metadata in the head and the content in `<main>` without placeholder or hidden blocks, missing pages answer 404, and the icons are valid PNG and ICO files. Unit tests cover the list metadata, the structured data, llms.txt, the ICO file and the IndexNow notice; the browser tests cover the head and body a crawler reads, the 404 status, the redirect of a renamed product, the icons and key, later pages, searches, the empty profile and the leaderboard's structured data. IndexNow and the host redirect act only in production; the host redirect was also tried on a production build with `VERCEL_ENV=production` (other hosts get a 308 with the path and query kept, the site's own host, `/api/` and `/_next/` do not), and the first real notices can be checked in Bing Webmaster Tools. `npm run check` (272 unit tests in 26 files), `npm run build` and `npm run test:e2e` (15 Chromium tests) pass.

Still to do by the owner, once: add `thesaasharbor.com` to Google Search Console as a Domain property (DNS TXT record), submit the sitemap, and import the site into Bing Webmaster Tools from Search Console (see README, Production). The site's findability also depends on links from elsewhere, such as launch sites and directories, which no code change replaces.
...
At the project owner's request, after a look at PublishYourSaaS, founders can prove that the product's website is theirs. Anyone could list a known product's address before and connect their own payment account; now the real owner can show the domain is theirs. The product editor has a Domain section with the TXT record to add (`_thesaasharbor.<domain>`, with a code for the product) and a Check DNS button. Verifying is optional: the details box on the product page says Domain: Verified or Not verified, as the owner asked on 2026-09-26 (first it was a line under the revenue figures), and the Markdown version says the same. TrustMRR has no such check: it ties a startup only to its payment account.

The record is looked up again every day by a new scheduled job (`/api/domains/check`, hourly); a record missing for three days, or a website on another domain, removes the mark. Founders check at most once every ten seconds per product and 30 times an hour. Only the server records a result, with the service-role key (migration `20260926100000_domain_verification.sql`, applied to production on 2026-09-26, before the code). pgTAP (`domain_verification.test.sql`), unit tests (the domain of an address, the lookup) and a browser test against a local DNS server cover it, with Axe scans of the editor in both themes and a check for sideways scrolling at 390 px.
...
10. Search engines: add the site to Google Search Console and Bing Webmaster Tools and submit the sitemap (README, Production), then check the home page and a product page with Google's Rich Results Test. Product pages describe a `SoftwareApplication` without a price or ratings, which Google needs for a rich result, so Search Console may list them as incomplete; that does not affect indexing. Private pages still stream under their placeholder and answer 200 with `noindex` for what is missing, which crawlers never see.
