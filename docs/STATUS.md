...
At the project owner's request, sign-up now has a required box: "I agree to the Terms of Service. I have read the Privacy Policy." The server refuses a sign-up without it, and the accepted version and time are recorded per account in `private.terms_acceptances` (migration `20260926020000_terms_acceptance.sql`), which account deletion removes; the privacy policy says so. The owner's production account predates the box and has no record. The browser test checks the box, the server's refusal and the recorded version; the whole path was also checked locally through Auth.
