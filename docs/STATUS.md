...
Visitors came, but none created an account, and almost all left from the home page, which showed only the leaderboard of demo products. At the project owner's request, the home page now invites every existing SaaS to list, in a compact header: the heading Get your SaaS seen. List it for free., the words that every SaaS is welcome, verified or not, with a public page, a link to the site and other founders who can write, and that connecting a payment provider is optional and puts it on the leaderboard, then a List your SaaS button and a link to How it works. Beside it, a dashed box shows the open place on the leaderboard (#1 is still free while no real product is ranked, then #N is next, until 12 are ranked), saying that listing needs no revenue check. The list follows under a slim Leaderboard heading. A first version with a large hero, three steps and four benefit cards took the whole first screen and looked generic; a second, a plain page header, was as dull as the original. A third, headed Prove your MRR, read as if verification were required, which it is not. All were replaced the same day.
