...
The website is not deployed. Production Supabase exists (see Production Supabase above); the app still needs a host, and the site an SMTP provider for the Auth and notification emails. Before launch, upgrade the Supabase organization to the Pro plan: free projects pause after a week without activity and have no daily backups.
...
4. Production Auth settings: all of them are in `supabase/config.toml` with `[remotes.production]` (the email templates and subjects, `https://thesaasharbor.com` as site URL and its `/auth/confirm` as redirect URL, 60 seconds between emails to one address, `secure_password_change`, a 12-character minimum password) and are applied with `npx supabase config push` once production SMTP is set (see README, Production). Still open: email rate limits for the SMTP provider and the CAPTCHA decision (see Release review).
