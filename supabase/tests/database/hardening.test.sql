-- Limits on revenue checks, image paths, and what account deletion removes from other makers'
...
insert into public.revenue_connections(saas_id, owner_id, provider, encrypted_key, key_hint,
  livemode, last_synced_at)
values ('e2000000-0000-4000-8000-000000000001', 'e1000000-0000-4000-8000-000000000001', 'stripe',
  'v1:x', 'rk_test_…abcd', false, now() - interval '1 hour');

-- Revenue checks: only for one's own product, a refresh at most every five minutes, and 20 an hour.
set local role authenticated;
select set_config('request.jwt.claim.sub', 'e1000000-0000-4000-8000-000000000002', true);
select throws_ok(
  $$ select public.begin_revenue_check('e2000000-0000-4000-8000-000000000001', false) $$,
  'P0001', 'You can only verify revenue for your own SaaS',
  'makers cannot check revenue for another maker''s product');
select set_config('request.jwt.claim.sub', 'e1000000-0000-4000-8000-000000000001', true);
select lives_ok($$ select public.begin_revenue_check('e2000000-0000-4000-8000-000000000001', true) $$,
  'a refresh an hour after the last one starts');
select throws_ok(
  $$ select public.begin_revenue_check('e2000000-0000-4000-8000-000000000001', true) $$,
...
select lives_ok($$ select public.begin_revenue_check('e2000000-0000-4000-8000-000000000001', false) $$,
  'connecting a new key is not held back by a recent refresh');
select lives_ok(
  $$ select public.begin_revenue_check('e2000000-0000-4000-8000-000000000001', false)
     from generate_series(1, 18) $$,
  'a maker checks revenue up to 20 times an hour');
select throws_ok(
  $$ select public.begin_revenue_check('e2000000-0000-4000-8000-000000000001', false) $$,
  'P0001', 'Revenue was checked many times in the last hour. Try again later',
  'the 21st check in an hour is refused');
select throws_ok('select count(*) from private.revenue_checks', '42501', null,
...
select is_empty(
  $$ select 1 from realtime.messages m
     ...
     where o.payload ->> 'sender_id' = 'e1000000-0000-4000-8000-000000000001'
     union all select 1 from private.revenue_checks k
  ...
  'no event, email, revenue check or product addition names the deleted maker');
