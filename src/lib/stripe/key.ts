...
export function parseRestrictedKey(
  ...
  if (!match) throw new Error("Paste a Stripe restricted key. It starts with rk_live_.");
  const livemode = match[1] === "live";
  if (!livemode && !allowTest) throw new Error("Use a live-mode key. Test keys are not accepted.");
