...
export async function listings({
  ...
  page = 1,
  owner,
}: { ranked?: boolean; category?: string; search?: string; page?: number; owner?: string } = {}) {
  const client = publicClient();
  if (!client)
    return {
      rows: [] as Listing[],
      count: 0,
      error: "Connect Supabase to open the harbor. See the setup guide in README.md.",
    };
  ...
  return {
    ...
    count: count ?? 0,
    error: error ? "The harbor could not be loaded. Please try again shortly." : null,
  };
}
