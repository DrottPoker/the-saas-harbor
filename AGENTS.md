...
- Development and tests use local Supabase in Docker. Production is the Supabase Cloud project `vgwgeennghaqpvfsqewq` ("The SaaS Harbor", eu-central-1). Write to it (`npx supabase db push --linked`, `npx supabase config push`, SQL) only with explicit approval and after the local checks pass, and never run tests, seeds, resets or `db:*` scripts against it. `config push` pushes without asking; see `[remotes.production]` in `supabase/config.toml`. Never point the local app at production.
