...
- Every exposed table needs RLS and explicit grants. Views use `security_invoker = true`. Only `src/lib/supabase/admin.ts` uses the service-role key, for Stripe verification writes and the email outbox worker; never use it for anything a maker controls directly.
