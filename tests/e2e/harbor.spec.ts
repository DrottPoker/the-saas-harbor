...
if (!url || new URL(url).hostname !== "127.0.0.1") throw new Error("Only the local test database is allowed.");
...
async function fillProduct(page: Page, name: string) {
  ...
  await page.getByLabel("One-line pitch").fill("A product created only by the local integration test.");
  await page.getByLabel("The story behind your SaaS").fill("This is an isolated local integration fixture and is removed after verification.");
...
test.afterAll(async () => {
  ...
  for (const user of data.users.filter(user => user.email === email || user.email === secondEmail)) {
    if (!userIds.includes(user.id)) userIds.push(user.id);
  }
  for (const id of userIds) {
    const { data: images } = await admin.storage.from("profile-images").list(id);
    if (images?.length) await admin.storage.from("profile-images").remove(images.map(image => `${id}/${image.name}`));
    const { error } = await admin.auth.admin.deleteUser(id);
    if (error) throw new Error("Local fixture cleanup failed.");
  }
});

test("anonymous navigation, private route protection and responsive empty state", async ({ page }) => {
  await page.goto("/");
  ...
  await page.screenshot({ path: "test-results/desktop-home.png", fullPage: true, caret: "initial" });
  const accessibility = await new AxeBuilder({ page }).withTags(["wcag2a", "wcag2aa", "wcag21aa"]).analyze();
  expect(accessibility.violations.map(issue => ({ id: issue.id, nodes: issue.nodes.map(node => node.target) }))).toEqual([]);
});

test("registration, email confirmation, profile and SaaS editing, storage, privacy and logout", async ({ page, request }) => {
  ...
  await expect.poll(async () => {
    ...
    const message = mailbox.messages.find((item: { To: { Address: string }[] }) => item.To.some(to => to.Address === email));
  ...
  if (!confirmation || !confirmation.startsWith("http://127.0.0.1:54321/")) throw new Error("A local confirmation link is required.");
  ...
  await page.getByLabel("A little about you").fill("An isolated maker profile for browser verification.");
  await page.getByLabel("Website", { exact: true }).fill("https://example.com");
  await page.getByLabel("Upload image").setInputFiles({ name: "invalid.png", mimeType: "image/png", buffer: Buffer.from("not an image") });
  ...
  await page.getByLabel("Upload image").setInputFiles({ name: "avatar.png", mimeType: "image/png", buffer: png });
  await page.getByRole("button", { name: "Save profile" }).click();
  ...
  await page.getByLabel("Upload image").setInputFiles({ name: "logo.png", mimeType: "image/png", buffer: png });
  ...
  const { data: publicData } = await anon.from("public_saas").select("*").eq("id", productId).single();
  ...
  await page.goto("/");
  await expect(page.getByRole("link").filter({ hasText: productName }).getByText("$1,234.56", { exact: true })).toBeVisible();
  await page.goto(`/dashboard/saas/${productId}`);
...
test("another owner cannot read private history, edit SaaS, or overwrite images", async ({ page }) => {
  const { data, error } = await admin.auth.admin.createUser({ email: secondEmail, password: secondPassword, email_confirm: true });
  ...
  const other = createClient(url, publicKey, { auth: { persistSession: false, autoRefreshToken: false } });
  await other.auth.signInWithPassword({ email: secondEmail, password: secondPassword });
  const { data: reports, error: readError } = await other.from("metric_reports").select("*").eq("saas_id", productId);
  expect(readError).toBeNull();
  expect(reports).toEqual([]);
  const { data: updated } = await other.from("saas").update({ name: "Hijacked" }).eq("id", productId).select();
  expect(updated).toEqual([]);
  const { error: uploadError } = await other.storage.from("profile-images").upload(`${firstUserId}/forged.png`, png, { contentType: "image/png" });
  expect(uploadError).not.toBeNull();
  const { data: original } = await anon.from("public_saas").select("name,mrr_cents").eq("id", productId).single();
...
test("password recovery confirms a local email link and accepts the new password", async ({ page, request }) => {
  await page.goto("/auth?mode=reset");
  await page.getByLabel("Email address").fill(email);
  await page.getByRole("button", { name: "Send reset link" }).click();
  await expect(page.getByText("If an account exists, you will receive a password reset link.")).toBeVisible();
  let resetId = "";
  await expect.poll(async () => {
    const mailbox = await (await request.get("http://127.0.0.1:54324/api/v1/messages")).json();
    const message = mailbox.messages.find((item: { Subject: string; To: { Address: string }[] }) => item.Subject.toLowerCase().includes("reset") && item.To.some(to => to.Address === email));
    resetId = message?.ID ?? "";
    return !!resetId;
  }).toBe(true);
  const message = await (await request.get("http://127.0.0.1:54324/api/v1/message/" + resetId)).json();
  const link = message.HTML.match(/href="([^"]*\/auth\/v1\/verify[^"]*)"/)?.[1]?.replaceAll("&amp;", "&");
  if (!link || !link.startsWith("http://127.0.0.1:54321/")) throw new Error("A local reset link is required.");
